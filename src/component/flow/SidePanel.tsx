...
export function SidePanel({
  ...
  return (
    ...
    >
      <div
        className={`flex items-center justify-between px-4 py-3.5 text-[13px] border-b font-medium ${isLight ? "text-black/80 border-black/5" : "text-white/90 border-white/5"}`}
      >
        <span>Nodes</span>
        <button
      ...
      <div className="flex flex-col gap-1.5 px-3 pb-4">
        {filteredPalette.map((item) => (
          <button
            key={item.label}
            className={`h-[36px] rounded-lg border px-3 flex items-center justify-between text-xs cursor-pointer transition-all ${isLight ? "border-black/5 bg-black/2 text-black/60 hover:bg-black/5 hover:border-black/10 hover:text-black/90" : "border-white/5 bg-white/2 text-white/70 hover:bg-white/10 hover:border-white/10 hover:text-white/90"}`}
            onClick={() => addNode(item)}
          >
            <span>{item.label}</span>
            <span
              className={`text-[10px] uppercase tracking-wider font-medium ${isLight ? "text-black/30" : "text-white/30"}`}
            >
              Add
            </span>
          </button>
